...
```
/plugin marketplace add hir4ta/claude-pulse
/plugin install pulse@hir4ta/claude-pulse
```
...
Claude Code を再起動すれば完了。Guardrail プリセットは初回セッション開始時に自動セットアップされます。
...
未設定の場合、coach の検索は FTS5 キーワード検索にフォールバックします。
...
|------|------|-------------|
| `SessionStart` | Session begins | Record session (silent) |
